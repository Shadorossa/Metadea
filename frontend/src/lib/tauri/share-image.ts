...
// inPicturesFolder opens the dialog in Pictures/Metadea.
export async function saveImageFile(
  dataUrl: string,
  defaultName: string,
  options: { inPicturesFolder?: boolean } = {},
): Promise<string | null> {
  return tauriCmd<string | null>('save_image_file', null, { dataUrl, defaultName, inPicturesFolder: options.inPicturesFolder ?? false });
}
