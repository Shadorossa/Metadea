...
export function initAvatar(
  ...
  async function renderAvatarPreview() {
    const custom = await getImage(STORAGE_KEYS.profileAvatarCustom);
  ...
  input.addEventListener('change', async () => {
    ...
    if (!await saveImage(STORAGE_KEYS.profileAvatarCustom, dataUrl)) {
  ...
  removeBtn.addEventListener('click', async () => {
    await removeImage(STORAGE_KEYS.profileAvatarCustom);
    renderAvatarPreview();
