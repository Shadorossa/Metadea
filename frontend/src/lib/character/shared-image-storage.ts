import { SHARED_IMAGE_WORKER_URL } from '../api/urls';
import { invoke } from '../tauri';
import { getT } from '../../i18n/runtime';

const msg = () => getT().character_editor;
...
async function readImageSource(sourceUrl: string): Promise<Blob> {
  ...
  if (!response.ok) throw new Error(msg().shared_image_source_read_error);
  const contentType = response.headers.get('Content-Type')?.split(';')[0].trim() ?? '';
  if (!contentType.startsWith('image/')) throw new Error(msg().shared_image_not_an_image);
  const declaredSize = Number(response.headers.get('Content-Length'));
  if (Number.isFinite(declaredSize) && declaredSize > MAX_SOURCE_BYTES) throw new Error(msg().shared_image_source_too_large);

  const reader = response.body?.getReader();
  if (!reader) throw new Error(msg().shared_image_source_read_error);
  ...
  while (true) {
    ...
    if (total > MAX_SOURCE_BYTES) {
      ...
      throw new Error(msg().shared_image_source_over_limit);
    }
    chunks.push(value);
  }
...
async function webpBlobFromImageSource(sourceUrl: string): Promise<Blob> {
  ...
  try {
    ...
    if (!context) throw new Error(msg().shared_image_webp_prepare_error);
    ...
    for (let attempt = 0; attempt < 4; attempt++) {
      ...
      if (!blob || blob.type !== 'image/webp') throw new Error(msg().shared_image_webp_encode_error);
  ...
  throw new Error(msg().shared_image_still_too_large);
}
...
export async function uploadImageToSharedCatalog(
  ...
  if (collection === 'character' && (!externalId || !/^character:[^/?#]+$/.test(externalId))) {
    throw new Error(msg().shared_image_missing_external_id);
  }
  ...
  try {
    ...
    if (isInlineImage || collection === 'character') {
      throw new Error(msg().shared_image_copy_failed, { cause: error });
    }
  ...
  if (!token) throw new Error(msg().shared_image_github_login_required);
  ...
  if (!response.ok) {
    const message = typeof result?.error === 'string' ? result.error : msg().shared_image_upload_failed.replace('{status}', String(response.status));
    throw new Error(message);
  }

  if (typeof result?.imageUrl !== 'string') throw new Error(msg().shared_image_worker_no_url);
  ...
  if (sharedUrl.origin !== new URL(SHARED_IMAGE_WORKER_URL).origin || (!isExpectedCharacterAlias && !isExpectedHashImage)) {
    throw new Error(msg().shared_image_worker_invalid_url);
  }
