...
import type { AppearanceRow, VoiceActorRow } from './character-editor-diff';
import type { CharacterDraft, CharacterEditorState } from './character-editor-state';
...
// Resolves to the editor's reducer state: `baseline` is exactly what was
// loaded, `draft` is the same plus the pending appearance seed (if any) —
// the one thing that starts out already "changed".
export async function loadCharacterEditorData(
  ...
  if (rawAppearances.length > 0) {
    resolved = await Promise.all(rawAppearances.map(async (a): Promise<AppearanceRow> => {
      ...
      if (!entry && cached) {
        ...
        await saveCatalogEntry(entry).catch(err => console.error('Failed to cache appearance catalog entry:', err));
      }
      return {
  ...
  return { baseline, draft: { ...baseline, appearances: resolvedWithPending } };
}
