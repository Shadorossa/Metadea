...
async function fetchTmdbUpcoming(rangeStart: Date, rangeEnd: Date): Promise<UpcomingRelease[]> {
  ...
  const MAX_PAGES = 3;
  // Every page takes a slot on TMDB's host budget (lib/api/rate-limiter.ts)
  // as a background request, so a search the user is typing at the same
  // time is served first.
  async function fetchPage<T>(url: string): Promise<T | null> {
    await acquireForUrl(url, 'background');
    return fetchJson<T>(url, { headers });
  }
  async function fetchKind(kind: 'movie' | 'tv', dateField: string): Promise<TmdbDiscoverItem[]> {
    const first = await fetchPage<{ results?: TmdbDiscoverItem[]; total_pages?: number }>(buildUrl(kind, dateField, 1));
    ...
    const rest = await Promise.all(
      Array.from({ length: remainingPages }, (_, i) =>
        fetchPage<{ results?: TmdbDiscoverItem[] }>(buildUrl(kind, dateField, i + 2))
      ),
    );
...
// ── localStorage cache ───────────────────────────────────────────────────────
// One slot per calendar month (see lib/home/upcoming-cache.ts for the
// format, the month cap and the freshness rule), so reopening Home, a full
// page reload, or stepping back and forth between adjacent months within
// UPCOMING_MIN_REFRESH_MS reuses the last fetch instead of re-hitting all
// three APIs. Not tied to any particular rangeStart/rangeEnd — it caches by
// month, independent of how "today" shifts the query boundaries.

function readCache(monthKey: string) {
  try {
    return readMonthFromCache(localStorage.getItem(STORAGE_KEYS.homeCalendarGeneralCache), monthKey, Date.now());
  } catch {
    return null;
  }
}
...
function writeCache(monthKey: string, releases: UpcomingRelease[]): void {
  try {
    ...
    localStorage.setItem(STORAGE_KEYS.homeCalendarGeneralCache, writeMonthToCache(raw, monthKey, releases, Date.now()));
  } catch {
    // Storage full/unavailable — cache is a pure optimization, safe to skip.
  }
}

// One network fetch per month at a time: toggling "General" off and on, or
// two month-arrow clicks that land on the same month, share the request
// that is already running instead of each firing their own.
const inFlight = new Map<string, Promise<UpcomingRelease[]>>();

function refreshMonth(monthKey: string, rangeStart: Date, rangeEnd: Date): Promise<UpcomingRelease[]> {
  const running = inFlight.get(monthKey);
  if (running) return running;
  const pending = fetchGeneralUpcomingReleasesUncached(rangeStart, rangeEnd)
    .then(releases => { writeCache(monthKey, releases); return releases; })
    .finally(() => { if (inFlight.get(monthKey) === pending) inFlight.delete(monthKey); });
  inFlight.set(monthKey, pending);
  return pending;
}
...
//
// Cached data first: a fresh cached month is returned as is; a stale one is
// returned immediately too while the refresh runs, and `onRefreshed` gets
// the fresh list once it lands (never called when nothing was cached — the
// returned promise is the fresh list then). rangeStart/rangeEnd should be
// midnight of the 1st and the last day of the month respectively.
export async function fetchGeneralUpcomingReleases(
  rangeStart: Date,
  rangeEnd: Date,
  onRefreshed?: (releases: UpcomingRelease[]) => void,
): Promise<UpcomingRelease[]> {
  const monthKey = monthKeyFor(rangeStart);
  const cached = readCache(monthKey);
  if (cached?.fresh) return cached.releases;

  const refresh = refreshMonth(monthKey, rangeStart, rangeEnd);
  if (!cached) return refresh;
  refresh.then(releases => onRefreshed?.(releases), () => {});
