...
import type { MediaCatalogEntry, CatalogEntryLike } from '../../tauri';
...
import { IN_PROGRESS_STATUSES, isReadingType } from '../media-types';
...
// Accepts a CatalogSummary projection too (the profile grid's editor
// placeholder) — every column outside it is optional here and simply
// maps to undefined, exactly as a full row with that column unset does.
export function mapCatalogEntryToPartialData(c: CatalogEntryLike, progressLabel: string = getT().media.progress_in_progress): MediaPageData {
  const tm = getT().media;
  ...
  return {
    ...
    issueSourceId: c.issue_source_id ?? undefined,
    episodeSourceId: c.episode_source_id ?? undefined,
...
export function mapMediaDataToCatalogEntry(data: MediaPageData, externalId: string): MediaCatalogEntry {
  ...
  return {
    ...
    issue_source_id:       data.issueSourceId || null,
    episode_source_id:     data.episodeSourceId || null,
