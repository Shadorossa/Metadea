...
export async function getAnimePrequelThemeOffsets(rawId: string): Promise<{ opOffset: number; edOffset: number }> {
  ...
  // Each prequel's themes are independent of the others; the offsets are
  // summed afterwards in chain order, so nothing observable depends on the
  // fetch order.
  const themesPerPrequel = await Promise.all(prequels.map(async prequel => {
    ...
    if (themes.length === 0) {
      ...
      if (pType === 'anime' && numericId) {
        ...
        if (remote.length > 0) {
          ...
          saveMediaThemes(prequel.externalId, themes).catch(err => console.error('Failed to cache prequel themes:', err));
        }
      }
    }
    return themes;
  ...
  for (const themes of themesPerPrequel) {
