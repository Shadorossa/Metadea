...
async function fetchSagaArcs(entries: SagaEntry[]): Promise<SagaArcsResult> {
  // Display-only (SagaViewerModal): arc images come back as file paths the
  // viewer wraps with wrapAssetUrl, never a data URL to re-save.
  const arcs = await getStoryArcsForMediaBatchLight(entries.map(e => e.externalId)).catch(() => [] as StoryArc[]);
