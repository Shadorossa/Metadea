...
// everything else as "Juego".
export const BUNDLE_ADDON_FORMATS: Record<string, string> = { DLC: 'DLC', EXPANSION: 'Expansión' };
...
export function hasBundleAddon(children: { format?: string | null }[]): boolean {
  return children.some(c => c.format && BUNDLE_ADDON_FORMATS[c.format]);
}
...
export function bundleChildLabel(child: { format?: string | null }, index: number, hasAddon: boolean): string {
  if (!hasAddon) return `Part ${toRoman(index + 1)}`;
  return (child.format && BUNDLE_ADDON_FORMATS[child.format]) || 'Juego';
}
