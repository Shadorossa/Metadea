...
import { getCachedMediaRelations } from '../../lib/profile/library-data-cache';
import { unifyTasteSeasons } from '../../lib/social/taste-seasons';
...
export function TasteHeartButton({ taste, catalogMap, theirName, theirAvatarUrl, theirBannerUrl, s }: HeartButtonProps) {
  ...
  // "Unify seasons": later AniList seasons collapse to the first one in the
  // cover lists (lib/social/taste-seasons.ts); needs the relation graph.
  const [relations, setRelations] = useState<DbMediaRelation[] | null>(null);
  const unify = isUnifySeasonsEnabled();
  useEffect(() => {
    if (!open || !unify || relations) return;
    let cancelled = false;
    getCachedMediaRelations().then(rows => { if (!cancelled) setRelations(rows); }).catch(() => { if (!cancelled) setRelations([]); });
    return () => { cancelled = true; };
  }, [open, unify, relations]);
  const shownTaste = useMemo(
    () => (taste && catalogMap && unify && relations ? unifyTasteSeasons(taste, relations, catalogMap) : taste),
    [taste, catalogMap, unify, relations],
  );
  ...
  return (
    <>
      ...
      {open && shownTaste && catalogMap && (!unify || relations) && (
