...
export function MediaSourceMappingSearchPopup({ kind, preferredIssueCount, onSelect, onClose }: Props) {
  ...
  return createPortal(
    <div className="pr-editor-search-popup" onClick={onClose}>
      <div className="pr-editor-search-popup-content pr-editor-search-popup-content--wide" onClick={event => event.stopPropagation()}>
        <div className="pr-editor-search-controls">
          <input
            autoFocus
            className="pr-editor-search-input"
            placeholder={kind === 'episodes' ? 'Buscar serie en TMDB…' : 'Buscar volumen en ComicVine…'}
            value={query}
