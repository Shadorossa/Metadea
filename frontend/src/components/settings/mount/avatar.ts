...
export function initAvatar(
  ...
  initImageUploadField({
    ...
    tooLargeMessage: getT().settings.avatar_too_large,
    saveErrorMessage: getT().settings.banner_save_error,
    showToast,
...
export function initShareAvatar(showToast: (msg?: string) => void) {
  ...
  async function openEditor(initialUrl: string) {
    const result = await openImageCropModal({
      title: getT().settings.share_avatar_title,
      initialUrl,
      aspectRatio: 1,
      saveLabel: getT().character_editor.photo_save,
    });
    if (result.action !== 'saved') return;
    const ok = await saveImage(STORAGE_KEYS.shareAvatarCustom, result.imageUrl);
    if (!ok) { showToast(getT().settings.banner_save_error); return; }
