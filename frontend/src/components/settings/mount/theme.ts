...
import { saveUserInfo } from '../../../lib/tauri';
import { getT } from '../../../i18n/runtime';
import type { Translations } from '../../../i18n/index';

type ThemeNameKey = keyof Translations['settings'];

const THEMES = [
  {
    id: 'nebula',
    nameKey: 'theme_nebula' as ThemeNameKey,
...
export function initThemePicker(showToast: (msg?: string) => void) {
  ...
  grid.innerHTML = THEMES.map(t => `
    <button class="theme-card${t.id === current ? ' active' : ''}" data-theme-id="${t.id}">
      ...
      <span class="theme-card-name">${'nameKey' in t ? getT().settings[t.nameKey] : t.name}</span>
    </button>
  `).join('');
