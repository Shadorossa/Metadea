...
#[tauri::command]
pub async fn read_user_lists(app_handle: tauri::AppHandle) -> Result<String, String> {
    let data_dir = app_handle
        .path()
        .app_data_dir()
        .map_err(|e| e.to_string())?;
    let path = data_dir.join("user_metadata").join("user_lists.json");
    if !path.exists() {
        return Ok("[]".to_string());
    }
    std::fs::read_to_string(path).map_err(|e| e.to_string())
